...
# 3. Atualizar schema do banco de dados (baseline + migração customizada 0251)
if [ -f "hostgator-setup-kit/_common.sh" ] && [ -f ".env" ]; then
  ...
  if [ -n "$DB_URL" ] && [ -f "supabase/baseline.sql" ]; then
    ...
    if [ -f "supabase/migrations/20260914210000_0251_openrouter_custom_models_publish.sql" ]; then
      echo "→ Aplicando migration 0251_openrouter_custom_models_publish..."
      docker run --rm -i -v "$ROOT_DIR/supabase/migrations/20260914210000_0251_openrouter_custom_models_publish.sql:/m.sql:ro" postgres:17-alpine psql "$DB_URL" -f /m.sql >/dev/null 2>&1 || true
    fi
    echo "✓ Banco de dados atualizado com sucesso."
  fi
fi

# 4. Assegurar que .env e .env.local usem a imagem customizada do app e versão 1.23.0 dos workers
...
echo "→ Puxando imagens oficiais atualizadas (worker e scheduler 1.23.0)..."
...
echo "=== [Deskcomm Custom] Concluído com sucesso! App v1.23.0 no ar com customizações OpenRouter. ==="
