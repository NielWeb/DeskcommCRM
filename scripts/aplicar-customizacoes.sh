...
# 3. Atualizar schema do banco de dados (baseline + migração customizada 0336)
if [ -f "hostgator-setup-kit/_common.sh" ] && [ -f ".env" ]; then
  ...
  if [ -n "$DB_URL" ] && [ -f "supabase/baseline.sql" ]; then
    ...
    if [ -f "supabase/migrations/20260919170000_0336_openrouter_custom_models_publish.sql" ]; then
      echo "→ Aplicando migration 0336_openrouter_custom_models_publish..."
      docker run --rm -i -v "$ROOT_DIR/supabase/migrations/20260919170000_0336_openrouter_custom_models_publish.sql:/m.sql:ro" postgres:17-alpine psql "$DB_URL" -f /m.sql >/dev/null 2>&1 || true
    fi
...
for ENV_FILE in .env .env.local; do
  if [ -f "$ENV_FILE" ]; then
    ...
    sed -i 's|^SCHEDULER_IMAGE=.*|SCHEDULER_IMAGE=ghcr.io/melgarafael/deskcomm-scheduler:1.38.0|' "$ENV_FILE"
  fi
done

# 5. Reconstruir imagens customizadas e atualizar contêineres
echo "→ Puxando scheduler oficial 1.38.0..."
